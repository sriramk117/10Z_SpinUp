...
void odom::moveForwardPID(double d, double cap) {
  ...
  double curr_pos = frontLeft.position(degrees) * ((double)36.0/60.0) + frontRight.position(degrees) * ((double)36.0/60.0); //+ frontRight.position(degrees) * ((double)36/60);//frontLeft.position(degrees) * (600/360);
  curr_pos = (curr_pos/(double)2.0);
  double dist = ((fabs(d)/(3.25*3.1415926535)) * 360.0) - abs(curr_pos);//sqrt(x*x + y*y) - sqrt(odom::x * odom::x + odom::y * odom::y);
  ...
  while(abs(dist) > 50) {
    ...
    cout << "Dist: " << dist << "\n";
    //curr_pos = frontLeft.position(degrees) * (600/360);
    curr_pos = frontLeft.position(degrees) * ((double)36.0/60.0) + frontRight.position(degrees) * ((double)36.0/60.0);//frontLeft.position(degrees) * (600/360);
    curr_pos = (curr_pos/(double)2.0);
    dist = ((fabs(d)/(3.25*3.1415926535)) * 360.0) - abs(curr_pos); //sqrt(x*x + y*y) - sqrt(odom::x * odom::x + odom::y * odom::y);
