...
void driver::robotDrive(){
  ...
  while (1) {
    ...
    double longitudinal = Controller1.Axis4.position(vex::percent);  //Axis 3 controls forward + backward
    //int lateral = Controller1.Axis4.position(vex::percent);       //Axis 4 controls strafe left + strafe right
    double rotational = Controller1.Axis3.position(vex::percent);    //Axis 1 controls turning
...
void driver::shooter() {
  ...
  if (Controller1.ButtonA.pressing()) {
    ...
    flywheel.spin(vex::reverse, 54, vex::percent);
  } else {
    flywheel.spin(vex::reverse, 54, vex::percent);
  }
